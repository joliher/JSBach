async function runAction(action, btn) {
    ...
    if (action === 'start' || action === 'stop' || action === 'restart' || action === 'status') {
        // Ejecutar la acción en el backend y esperar el resultado antes de navegar
        try {
            ...
            // Guardar resultado breve para que la página de estado lo muestre
            try { sessionStorage.setItem('firewallLastAction', JSON.stringify({ action, result: data })); } catch (e) {}
        } catch (error) {
            console.error('Error al ejecutar la acción:', error);
            try { sessionStorage.setItem('firewallLastAction', JSON.stringify({ action, result: { success: false, message: error.message } })); } catch (e) {}
        }

        // Navegar a la página de estado para mostrar feedback
...
async function checkDependencies() {
    ...
    try {
        ...
        const vlansDiv = document.getElementById('dep-vlans');
        if (vlansDiv) {
            vlansDiv.innerHTML = '⚠️ VLANs: Error al verificar';
        }
    }
