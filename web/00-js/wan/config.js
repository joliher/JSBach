...
function getNetworkInfo(ip, mask) {
    try {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Marcar cuando el usuario empieza a editar el formulario para evitar recargas automáticas
    ['interface','mode','ip','mask','gateway','dns'].forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('input', () => { userEditing = true; });
            el.addEventListener('change', () => { userEditing = true; });
    ...
    async function loadWanConfiguration() {
        try {
            ...
            if (response.ok) {
                ...
                // Pre-rellenar el formulario con la configuración actual solo si el usuario NO está editando
                if (!userEditing) {
                    if (wanInterface !== "No configurada") {
                        const ifaceEl = document.getElementById("interface");
                        if (ifaceEl && !ifaceEl.value.trim()) ifaceEl.value = wanInterface;
                    }

                    if (wanMode === "manual" || wanMode === "dhcp") {
                        const modeEl = document.getElementById("mode");
                        if (modeEl && !modeEl.value) modeEl.value = wanMode;
                        manualFields.classList.toggle("hidden", wanMode !== "manual");

                        if (wanMode === "manual") {
                            const ipEl = document.getElementById("ip");
                            if (ipEl && !ipEl.value) ipEl.value = wanConfig.ip || "";
                            const maskEl = document.getElementById("mask");
                            if (maskEl && !maskEl.value) maskEl.value = wanConfig.mask || "";
                            const gwEl = document.getElementById("gateway");
                            if (gwEl && !gwEl.value) gwEl.value = wanConfig.gateway || "";
                            const dnsEl = document.getElementById("dns");
                            if (dnsEl && !dnsEl.value) dnsEl.value = wanConfig.dns || "";
                        }
                    }
                }
            } else {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if (mode === "manual") {
            ...
            const netInfo = getNetworkInfo(ip, mask);
            if (netInfo) {
                if (netInfo.ipNum === netInfo.networkNum || netInfo.ipNum === netInfo.broadcastNum) {
                    return showError(`❌ IP inválida: ${ip} es una dirección de red o broadcast con máscara /${mask}`);
                }

                // Validar Gateway en la misma red
                const gwInfo = getNetworkInfo(gateway, mask);
                if (gwInfo.networkNum !== netInfo.networkNum) {
                    return showError(`❌ Gateway fuera de red: ${gateway} no pertenece a la red de ${ip}/${mask}`);
                }
            }
        }
