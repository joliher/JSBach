...
def initialize_default_vlans(config_file: str) -> None:
    ...
    cfg["vlans"] = vlans
    success = save_json_config(config_file, cfg)
    if not success:
        # Si no se guardó la configuración, emitir una advertencia y no crashear
        print(f"Advertencia: No se pudo guardar configuración de VLANs por defecto en {config_file}")
