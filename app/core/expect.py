...
def _run_expect_script(script_content: str, timeout: int = 10, env_vars: Optional[Dict[str, str]] = None) -> Tuple[bool, str, str]:
    ...
    Args:
        script_content: Contenido del script .exp
        timeout: Timeout en segundos
        env_vars: Diccionario de variables de entorno para pasar al subproceso (para passwords)
    ...
    try:
        ...
        # Ejecutar expect
        cmd = ['/usr/bin/expect', script_path]
        ...
        process = subprocess.run(
            ...
            text=True,
            timeout=timeout,
            env=env  # Pasar variables de entorno con secretos
        )
        ...
        return False, "", "Timeout ejecutando script de expect"
...
def _generate_ssh_script(host, user, port, commands, timeout=30):
    """Genera el script de expect para SSH usando env var para password."""
    # Nota: El password se pasará por variable de entorno 'SSH_PASS'
    
    # Construir script
    script = f"set timeout {timeout}\n"
    script += f'set host "{host}"\n'
    script += f'set user "{user}"\n'
    script += f'set port {port}\n'
    script += '# Password desde variable de entorno\n'
    script += 'set password $env(SSH_PASS)\n\n'
    
    script += 'spawn ssh -p $port $user@$host\n'
    script += 'expect {\n'
    script += '    "yes/no" { send "yes\\r"; exp_continue }\n'
    script += '    "password:" { send "$password\\r" }\n'
    script += '    timeout { exit 1 }\n'
    script += '}\n\n'

    script += 'expect {\n'
    script += '    # Prompt genérico\n'
    script += '    -re {[#>$]} { }\n'
    script += '    timeout { exit 1 }\n'
    script += '}\n'
...
def _generate_telnet_script(host, user, port, commands, timeout=30):
    """Genera el script de expect para Telnet usando env var para password."""
    # Nota: El password se pasará por variable de entorno 'TELNET_PASS'
    
    script = f"set timeout {timeout}\n"
    script += f'set host "{host}"\n'
    script += f'set user "{user}"\n'
    script += f'set port {port}\n'
    script += 'set password $env(TELNET_PASS)\n\n'

    script += 'spawn telnet $host $port\n'
    script += 'expect {\n'
    script += '    "Login:" { send "$user\\r"; exp_continue }\n'
    script += '    "login:" { send "$user\\r"; exp_continue }\n'
    script += '    "Username:" { send "$user\\r"; exp_continue }\n'
    script += '    "Password:" { send "$password\\r" }\n'
    script += '    "password:" { send "$password\\r" }\n'
    script += '    timeout { exit 1 }\n'
    script += '}\n\n'
...
def execute_commands(target_config: Dict[str, Any], commands: List[str]) -> Tuple[bool, str]:
    """Ejecuta una lista de comandos en un objetivo."""
    ip = target_config.get("ip")
    protocol = target_config.get("protocol", "ssh")
    port = target_config.get("port", 22 if protocol == "ssh" else 23)
    user = target_config.get("username")
    password = target_config.get("password")
    
    if not all([ip, user, password]):
        return False, "Faltan credenciales (ip, user, password)"
    ...
    try:
        ...
        if success:
            ...
            return False, f"Error ejecución: {stderr if stderr else stdout}"
            
    except Exception as e:
        logger.exception(f"Error ejecutando comandos en {ip}")
        return False, str(e)
...
def config(params: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if auth_required and user:
        ...
        script_lines.append("send \"$env(EXPECT_PASS)\\r\"")
    ...
    # 6. Ejecutar Script con seguridad
    ...
    if success:
        ...
        logger.error(f"Error ejecutando expect en {ip}: {stdout} {stderr}")
        
        # Sanitizar errores críticos para el usuario
        combined_msg = (stdout + " " + stderr).lower()
...
def reset(params: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if success:
        ...
        logger.error(f"Error en reset: {stdout} {stderr}")
        return False, f"Error en reset: {stdout if stdout else stderr}"
