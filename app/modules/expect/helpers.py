...
def get_switch_auth(ip: str) -> Dict[str, str]:
    """Obtiene credenciales formateadas para un switch."""
    # Buscar en la ruta estándar de la app
    from .state_manager import BASE_DIR
    secrets_json = os.path.join(BASE_DIR, "config", "expect", "secrets.json")
    user, password = get_secrets(ip, secrets_json)
    return {"user": user or "admin", "password": password or ""}
...
def normalize_mac(raw: str) -> str:
    """Normaliza una dirección MAC al formato HH:HH:HH:HH:HH:HH."""
